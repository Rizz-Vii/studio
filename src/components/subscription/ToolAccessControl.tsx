...
export function ToolCategoryView({ showRestricted = true }: ToolCategoryViewProps) {
          ...
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
            {category.tools.map((tool) => (
              ...
              >
                ...
                {!tool.available && showRestricted && tool.restrictedMessage && (
                  <p className="text-sm text-gray-600 mt-2">{tool.restrictedMessage}</p>
                )}
              </div>
            ))}
          </div>

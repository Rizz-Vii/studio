...
const AuditCharts = ({ items }: { items: AuditUrlOutput['items']}) => {
    const chartData = items.map(item => ({ name: item.name, score: item.score, fill: item.score > 85 ? 'hsl(var(--chart-1))' : item.score > 60 ? 'hsl(var(--chart-2))' : 'hsl(var(--chart-5))' }));
    ...
    if (imageAuditItem) {
        ...
        const match = imageAuditItem.details.match(/(\d+)\s*images.*(\d+)\s*are\s*missing/);
        if (match) {
        ...
        <div className="space-y-6">
            ...
            {imageData && (
                 <Card>
                    ...
                    <CardContent>
                        <ChartContainer config={imageChartConfig} className="mx-auto aspect-square h-[200px]">
                            <PieChart>

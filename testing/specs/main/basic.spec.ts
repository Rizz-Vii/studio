...
test.describe("Basic Health Check", () => {
  test("can connect to the application", async ({ page }) => {
    ...
    try {
      ...
      await page.screenshot({ path: "test-results/basic-health-check.png", fullPage: true });
  ...
  test("can access login page", async ({ page }) => {
    ...
    try {
      ...
      await utils.waitForElementGracefully("#password, input[type='password'], input[id='password']", { 
        timeout: 20000, 
        retries: 3 
      });

      console.log("🔍 Looking for login button...");
      await utils.waitForElementGracefully("button[type='submit'], button:has-text('Login')", { 
        timeout: 20000, 
        retries: 3 
      });

...
export const TEST_USERS: Record<string, TestUser> = {
  ...
  starter: {
    ...
    description:
      "Starter tier user with intermediate features (50 audits/month)",
  },
...
export class UserManager {
  ...
  async loginAs(userType: keyof typeof TEST_USERS): Promise<TestUser> {
    ...
    console.log(
      `🔐 Logging in as ${user.displayName} (Role: ${user.role}, Tier: ${user.tier})`
    );
  ...
  private checkFeatureAccess(
    userAccess: { role: UserRole; tier: SubscriptionTier; status: string },
    featureName: string
  ): boolean {
    // Admin-only features
  ...
  getExpectedFeatures(): string[] {
    ...
    return allFeatures.filter((feature) =>
      this.checkFeatureAccess(userAccess, feature)
    );
  }
  ...
  async verifyUserRole(expectedRole: string): Promise<boolean> {
    try {
      ...
      const roleIndicators = await this.page
        .locator(
          [

...
test.describe("Free Tier User Tests", () => {
  ...
  test("Free User - Complete Dashboard Flow", async ({ page }) => {
    const dashboardFlow = freeUserFlows.find((flow) =>
  ...
  test("Free User - Keyword Tool Functionality", async ({ page }) => {
    const keywordFlow = freeUserFlows.find((flow) =>
      flow.name.includes("Keyword Tool")
    );
    if (!keywordFlow) {
  ...
  test("Free User - Feature Access Restrictions", async ({ page }) => {
    const restrictionFlow = freeUserFlows.find((flow) =>
      flow.name.includes("Feature Access")
    );
    if (!restrictionFlow) {
